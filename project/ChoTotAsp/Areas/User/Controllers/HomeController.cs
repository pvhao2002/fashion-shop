...
namespace ChoTotAsp.Areas.User.Controllers
{
    public class HomeController : System.Web.Mvc.Controller
    {
        ...
        public async Task<ActionResult> Index(string act)
        {
            using(var ctx = new DBConnection())
            {
                ...
                var listCategory = await ctx.categories
                    ...
                    .ToListAsync();
                var listProduct = await ctx.products
                    .Include(item => item.product_images)
                    .Include(item => item.category)
                    .Where(item =>
                        Constant.APPROVED.Equals(item.status)) // approved la da duoc admin kiem duyet
                ...
                TempData[Constant.STATUS_RS] = act; // neu act = open_login thi mo form login ra khi load trang home
                return View(model);
            }
        }
        ...
        public async Task<JsonResult> Login(string email, string password)
        {
            using(var ctx = new DBConnection())
            {
                var user = await ctx.users.FirstOrDefaultAsync(u => u.email.Equals(email));
                ...
                if (Constant.INACTIVE.Equals(user.status))
                {
                    return Json(new { succeess = false, message = "Tài khoản của bạn đã bị khóa" });
                }
                ...
                Session[Constant.USER_ID] = user.user_id;

                // Set cookie
                var cookieUserId = new HttpCookie(Constant.USER_ID, user.user_id.ToString())
                {
                    Expires = DateTime.Now.AddDays(MaxAge)
                };
                Response.Cookies.Add(cookieUserId);

                return Json(new { success = true, area = user.role });
            }
        }

        public async Task<ActionResult> ExternalLoginCallback(string returnUrl)
        {
            var loginInfo = HttpContext.GetOwinContext().Authentication.User;
            if (loginInfo != null) return await RedirectToLocal(returnUrl, loginInfo);
            TempData[Constant.STATUS_RS] = Constant.OPEN_LOGIN;
            return RedirectToAction("Index");
        }


        private async Task<ActionResult> RedirectToLocal(string returnUrl, ClaimsPrincipal loginInfo)
        {
            using(var ctx = new DBConnection())
            {
                ...
                var email = loginInfo.FindFirst(ClaimTypes.Email)?.Value;
                var accountExist = await ctx.users.FirstOrDefaultAsync(a => a.email.Equals(email));

                if (accountExist == null)
                {
                    var avatar = loginInfo.FindFirst("picture")?.Value ??
                    ...
                    {
                        full_name = loginInfo.Identity.Name,
                        email = email,
                        password_hash = "",
                        avatar = avatar,
                    ...
                    try
                    {
                        accountExist = ctx.users.Add(account);
                        await ctx.SaveChangesAsync();
                    }
                    catch (Exception e)
                    {
                        return Content("<script>window.close(); showAlert('fail', 'danger');</script>");
                    }
                }

                Session[Constant.USER_ID] = accountExist.user_id;

                // Set cookie
                var cookieUserId = new HttpCookie(Constant.USER_ID, accountExist.user_id.ToString())
                {
                    Expires = DateTime.Now.AddDays(MaxAge)
                };
                Response.Cookies.Add(cookieUserId);

                return Content("<script>window.close(); window.opener.location.href='/User/Home';</script>");
            }
        }
    }
}